...
## Kdocs Skill
Kdocs file URLs (`365.kdocs.cn/l/...` or `wps.cn/l/...`) render heavy HTML. The proxy cascade often returns partial or nav-heavy content. Use kdocs-cli for Kdocs file handling.

### One-time install:

```bash
npx skills add kdocs-app/kdocs-skill -g -y
bash ~/.agents/skills/kdocs/scripts/setup.sh
```

### Reading a shared link
extract `link_id` from the URL path (`/l/<link_id>`) and call directly:

```bash
kdocs-cli drive read-file-content '{"link_id":"<link_id>","format":"markdown"}' --silent
```

The response JSON contains the Markdown in `data.markdown`. No need to call `get-share-info` first.

### Fallback
Use `kdocs-cli` for help if `read-file-content` failed:

```bash
kdocs-cli --help # find a available way for file handing
```

or use `/kdocs` skill directly:

```bash
/kdocs 读取 365.kdocs.cn/l/... 文件内容，以markdown形式返回。
```
